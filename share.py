...
class ShareServer(ThreadingHTTPServer):
    ...
    def handle_error(self, request, client_address):
        ...
        self._print_error(
            f'{year:04}/{month:02}/{day:02} {hh:02}:{mm:02}:{ss:02} - {client_address[0]}:{client_address[1]} - {t.__name__}: {value}')
...
class BaseHandler(BaseHTTPRequestHandler):
    ...
    31, 92, 127, 128, 129, 130, 131, 132, 133, 134, 135, 136, 137, 138, 139, 140, 141, 142, 143, 144, 145, 146, 147,
    ...
    def _build_html_for_password(self):
        ...
        builder.append(
            '<input name="password" type="password" placeholder="Enter your password" minlength="3" required autofocus>')
...
''')
        ...
        for f in files:
            ...
            builder.append(
                '<svg xmlns="http://www.w3.org/2000/svg" enable-background="new 0 0 24 24" height="20px" viewBox="0 0 24 24" width="20px" fill="#2965c7"><g><path d="M18,15v3H6v-3H4v3c0,1.1,0.9,2,2,2h12c1.1,0,2-0.9,2-2v-3H18z M17,11l-1.41-1.41L13,12.17V4h-2v8.17L8.41,9.59L7,11l5,5 L17,11z"/></g></svg>')
            builder.append('</a>')
            builder.append('</span>')
            builder.append('</li>')
        builder.append('</ul>')
    ...
    def _is_hidden_windows(self, file_path):
        try:
            return self._is_hidden_unix(file_path) or os.stat(
                file_path).st_file_attributes & stat.FILE_ATTRIBUTE_HIDDEN != 0
        except FileNotFoundError:
            return False
...
class FileShareHandler(BaseFileShareHandler):
    ...
    def do_get(self):
        ...
        try:
            if path == '/':
                files = sorted(
                    FileItem(os.path.basename(f), self.is_hidden(f), os.path.getsize(f)) for f in self._files)
...
def start_server(address, port, certfile, keyfile, keypass, handler_class):
    ...
    with ShareServer(addr, handler_class) as server:
        ...
        sys.stderr.write(f'Serving HTTP on {host}:{port} ...\n')
...
def main():
    ...
    general.add_argument('-r', '--receive', action='store_true',
    ...
    general.add_argument('-P', '--password', nargs='?', const=os.getenv('SHARE_PASSWORD'),
    ...
    start_server(socket.gethostbyname(socket.gethostname()), args.port, args.certfile, args.keyfile, args.keypass,
                 handler_class)
